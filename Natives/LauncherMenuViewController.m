...
- (void)viewDidLoad {
    ...
    PLApplyToolbarAppearance(self.navigationController.toolbar);
...
- (UIBarButtonItem *)drawAccountButton {
    if (!self.accountBtnItem) {
        ...
        self.accountButton.contentEdgeInsets = UIEdgeInsetsMake(8, 10, 8, 12);
        self.accountButton.backgroundColor = [PLCardColor() colorWithAlphaComponent:0.92];
        self.accountButton.layer.cornerRadius = 18.0;
        if ([self.accountButton.layer respondsToSelector:@selector(setCornerCurve:)]) {
        ...
        self.accountButton.layer.borderColor = [UIColor.separatorColor colorWithAlphaComponent:0.18].CGColor;
        self.accountButton.tintColor = UIColor.labelColor;

        self.accountButton.titleEdgeInsets = UIEdgeInsetsMake(0, 6, 0, -6);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (origImage) {
        ...
        PLStyleAvatarView(cell.imageView, 12.0);
    }
...
- (void)updateAccountInfo {
    ...
    if (selected == nil) {
        ...
        PLStyleAvatarView(self.accountButton.imageView, 14.0);
        return;
    }
